...
basis.fun <- function(covars, cut.one.way, dt, treatment=NULL,
                      ...
                      cut.one.way.time=cut.one.way,
                      two.way=cbind(var1="", var2=""), cut.two.way=5,
                      cut.time.treatment=NULL, cut.time.covar=NULL, predict=FALSE, pseudo.dt=NULL, 
        ...
        model.matrix(formula(paste0(
            ...
            ifelse(length(time.var)>0 & cut.time.covar>0,
                   paste0("+", paste0(sapply(
                                   covars, function(covar) paste0(paste0(indicator.basis.fun(pseudo.dt, "grid.period", cut.time.covar), ":", indicator.basis.fun(pseudo.dt, covar, cut.one.way.time), "+"), collapse="")), collapse="+"), ""), ""),
