contmle <- function(dt,
                    #-- outcome model;
                    estimation=list("outcome"=list(fit=c("sl", "sl", "hal", "km", "rf"),
                                                   model=Surv(time, delta==1)~A+L1+L2+L3,#A+L1.squared,
                                                   changepoint=NULL),
                                    "cens"=list(fit=c("sl", "sl", "hal", "km", "rf"),
                    ...
                    browse.hal=FALSE,
                    cut.covars=15, cut.time=10,
                    cut.time.A=10,
                    cut.time.covar=NULL,
                    cut.L.A=8, cut.two.way=5,
                    hal.sl=FALSE,
                    cut.time.grid=c(15, 18, 25, 30),#(7:30)[seq(1,length(5:30),length=10)],
                    cut.covar.grid=c(15, 18, 25, 30),#(7:30)[seq(1,length(5:30),length=10)],
                    cut.two.way.grid=c(10, 15, 18, 25),#(7:30)[seq(1,length(5:30),length=10)],
                    ...
                    output.km=FALSE, only.km=FALSE, only.cox.sl=FALSE, output.RR=FALSE, output.bhaz=FALSE,
                    #-- dont use;
    ...
    if (dt[, length(unique(get(A.name)))]>2) {
        a <- treat.effect
    } else {
    ...
    if (dt[, length(unique(get(A.name)))]>2) {
        dt[, (paste0(A.name, ".num")):=as.numeric(get(A.name))]
        prob.A <- fit.categorical(as.formula(paste0(A.name, ".num", "~", as.character(treat.model)[3])),
                                  data=dt, pseudo.out=TRUE)[[2]]
        prob.A <- sapply(1:nrow(dt), function(ii) prob.A[ii, dt[ii, get(paste0(A.name, ".num"))]])        
    } else {
        prob.A <- predict(glm(as.formula(treat.model), data=dt, family=binomial), type="response")
    }

    if (verbose & dt[, length(unique(get(A.name)))]==2) print(summary(glm(as.formula(treat.model), data=dt)))
    if (verbose & dt[, length(unique(get(A.name)))]==2) print(summary(prob.A))
    ...
    for (each in 1:length(estimation)) {
        ...
        if (fit[1]=="sl" | fit[1]=="cox.hal.sl") { #-- cox-sl
            ...
            if (verbose.sl & verbose) print(fit.model)
            ...
            if (fit[1] %in% c("km")) { #-- later for hal or if uses km
                ...
                if (fit[1]=="km") tmp.model[3] <- paste0("strata(", A.name, ")") else tmp.model[3] <- "1"
        ...
        if (length(fit.changepoint)>0) { #-- if there is a change-point:
            ...
            if (fit[1]=="sl" & length(grep("coxnet", sl.pick))>0) {
                ...
                if (fit.delta==0) {
                    fit.cox <- rfsrc(as.formula(paste0(deparse(fit.model), collapse="")), data=copy(dt)[, (delta.var):=1*(dt[[delta.var]]==fit.delta)],
                                     #ntree=1, nodedepth=0, samptype="swr", bootstrap="none",                                     
                                     seed=rf.seed)
                } else {                
                    fit.cox <- rfsrc(as.formula(paste0(deparse(fit.model), collapse="")), data=dt,
                                     #ntree=1, nodedepth=0, samptype="swr", bootstrap="none",                                     
                                     seed=rf.seed)
                }
            } else {
        ...
        if (fit[1]=="km") {
            ...
            setnames(tmp, "strata", A.name)
            tmp[, (A.name):=as.numeric(gsub(paste0(A.name,"="), "", get(A.name)))]
            bhaz.cox <- merge(bhaz.cox, 
                              rbind(do.call("rbind", lapply(a, function(aa) data.table(time=0, hazard=0)[, (A.name):=aa])),
                                    tmp),
                              by=c("time", A.name), all.x=TRUE)
            ...
            if (fit[1]=="sl" & length(grep("coxnet", sl.pick))>0) { 
                ...
                bhaz.cox <- unique(merge(bhaz.cox, do.call("rbind", lapply(a, function(aa) {
                    rbind(data.table(time=0, hazard=0),
                          data.table(time=unique(dt[, get(time.var)]), hazard=0))[, (A.name):=aa]})),
                    by=c("time", A.name), all.x=TRUE), by=c("time", A.name))
                #unique(dt[dt[[delta.var]]>0, get(time.var)])
            } else {
                bhaz.cox <- merge(bhaz.cox, rbind(data.table(time=0, hazard=0),
    ...
    if (output.bhaz) {
        ...
        for (covar in covars2) 
            bhaz.cox[, (covar) := 0]

        bhaz.out <- list(bhaz.cox)

        for (each in 1:length(estimation)) {
        
            fit.cox <- estimation[[each]][["fit.cox"]]

            bhaz.out[[length(bhaz.out)+1]] <- fit.cox

            if (length(estimation[[each]][["changepoint"]])>0) { 
                bhaz.cox[, (paste0("period", estimation[[each]][["event"]])):=
                              (get(time.var)<=estimation[[each]][["changepoint"]])*1+(get(time.var)>estimation[[each]][["changepoint"]])*2]
            }

            bhaz.cox[, (paste0("exp", estimation[[each]][["event"]])) := exp(predict(fit.cox, newdata=bhaz.cox, type="lp"))]
        }

        names(bhaz.out) <- c("", sapply(1:length(estimation), function(each) estimation[[each]][["event"]]))
    ...
    for (each in 1:length(estimation)) {
        ...
        if (estimation[[each]][["fit"]][1]=="rf") {
            estimation[[each]][["predict.rf"]] <- predict(estimation[[each]][["fit.cox"]], newdata=dt2.a)
        }
        ...
        } else if (estimation[[each]][["fit"]][1]=="rf") {            
            dt2.a[, (paste0("fit.cox", estimation[[each]][["event"]])):=1]
        } else {
            dt2.a[, (paste0("fit.cox", estimation[[each]][["event"]])):=
    ...
    mat <- do.call("rbind", lapply(1:n, function(i) {
        ...
        for (each in 1:length(estimation)) {
            if (length(estimation[[each]][["changepoint"]])>0) {
                ...
                #prf <- predict(estimation[[each]][["fit.cox"]], newdata=dt2.a[id==i])
                #browser()
                prf <- estimation[[each]][["predict.rf"]]
                #prf <- predict(estimation[[each]][["fit.cox"]], newdata=dt2.a[id==i])
                if (cr & estimation[[each]][["event"]]!=0) {
                    chf <- prf$chf[,,estimation[[each]][["event"]]][(1:nrow(dt2.a))[dt2.a$id==i],]
                } else {
                    chf <- prf$chf[(1:nrow(dt2.a))[dt2.a$id==i],]
                }
                if (length(dim(chf))>0) {
                    tmp <- merge(#data.table()[, (time.var):=prf$forest$time.interest][, (paste0("dhaz", estimation[[each]][["event"]])):=dcf],
                        do.call("rbind", lapply(1:nrow(chf), function(kk) data.table()[, (time.var):=prf$forest$time.interest][, (A.name):=dt2.a[id==i][[A.name]][kk]][, (paste0("dhaz", estimation[[each]][["event"]])):=diff(c(0, chf[kk,]))])),
                        tmp[, -paste0("dhaz", estimation[[each]][["event"]]), with=FALSE], by=c(time.var, A.name), all.y=TRUE)
                    tmp[is.na(tmp[[(paste0("dhaz", estimation[[each]][["event"]]))]]), (paste0("dhaz", estimation[[each]][["event"]])):=0] 
                } else {
                    dcf <- diff(c(0, chf))
                    tmp <- merge(data.table()[, (time.var):=prf$forest$time.interest][, (paste0("dhaz", estimation[[each]][["event"]])):=dcf],
                                 tmp[, -paste0("dhaz", estimation[[each]][["event"]]), with=FALSE], by=c(time.var), all.y=TRUE)
                }
                ## tmp <- merge(#data.table()[, (time.var):=prf$forest$time.interest][, (paste0("dhaz", estimation[[each]][["event"]])):=dcf],
                ##     do.call("rbind", lapply(1:nrow(chf), function(kk) data.table()[, (time.var):=prf$forest$time.interest][, (A.name):=dt2.a[id==i][[A.name]][kk]][, (paste0("dhaz", estimation[[each]][["event"]])):=diff(c(0, chf[kk,]))])),
                ##     tmp[, -paste0("dhaz", estimation[[each]][["event"]]), with=FALSE], by=c(time.var, A.name), all.y=TRUE)
                tmp[is.na(tmp[[(paste0("dhaz", estimation[[each]][["event"]]))]]), (paste0("dhaz", estimation[[each]][["event"]])):=0]
                tmp[, (paste0("chaz", estimation[[each]][["event"]])):=cumsum(tmp[[(paste0("dhaz", estimation[[each]][["event"]]))]])]
                tmp[, (paste0("fit.cox", estimation[[each]][["event"]])):=1]
                #browser()
                if (estimation[[each]][["event"]]==0) {
                    tmp[, chaz0.1:=c(0, chaz0[-.N])]
                }
                if (estimation[[each]][["event"]]==0) {
                    if (length(dim(chf))>0) {
                        tmp <- merge(do.call("rbind", lapply(1:nrow(chf), function(kk)
                            data.table()[, (time.var):=prf$forest$time.interest][, (A.name):=dt2.a[id==i][[A.name]][kk]][, surv.C:=prf$survival[(1:nrow(dt2.a))[dt2.a$id==i],][kk,]])),
                            tmp, by=c(time.var, A.name), all.y=TRUE)
                    } else {
                        tmp <- merge(data.table(time=prf$forest$time.interest, surv.C=prf$survival[i,]),
                                     tmp, by=c(time.var), all.y=TRUE)
                    }
                }
                if (FALSE) {
                    print(paste0("length unique time.var = ",  length(unique(tmp[[time.var]]))))
                    tmp <- merge(data.table()[, (time.var):=prf$forest$time.interest][, (paste0("chaztest", estimation[[each]][["event"]])):=chf],
                                 tmp[, -paste0("dhaz", estimation[[each]][["event"]]), with=FALSE], by=time.var, all.y=TRUE)
                    tmp[time==0, chaztest1:=0]
                    tmp[, chaztest1:=na.locf(chaztest1)]
                    print(tmp[, all.equal(chaztest1, chaz1)])
                    tmp <- merge(data.table()[, (time.var):=prf$forest$time.interest][, (paste0("chaztest", estimation[[each]][["event"]])):=chf],
                                 tmp[, -paste0("dhaz", estimation[[each]][["event"]]), with=FALSE], by=time.var, all.y=TRUE)
                    tmp[, plot(chaztest1, chaz1)]
                }
            } else {
        ...
        if (any(unlist(lapply(estimation, function(x) x[["event"]]))==0)) {
            if (estimation[[(1:length(estimation))[unlist(lapply(estimation, function(x) x[["event"]]))==0]]][["fit"]][1]=="rf") {
                tmp[, surv.C1:=c(1,surv.C[-.N])]
                tmp[, surv.C1:=na.locf(surv.C1)]
            } else {
                tmp[, surv.C1:=exp(-fit.cox0*chaz0.1)]
            }
            if (tmp[, is.na(mean(is.na(surv.C1)))]) browser()
    ...
    if (FALSE) {
        test.km <- summary(prodlim(Hist(time, event2==1)~1, data=follic[, event2:=1*(event==0)]),
                           times=mat[, unique(time)], asMatrix=TRUE)$table
        test.km2 <- merge(mat, test.km, by="time")
        test.km2[, surv.1:=c(1, surv[-.N]), by="id"]
        test.km2[id==1, lm(surv.1~surv.C1)]
        test.km2[, all.equal(surv.1, surv.C1)]
    }

    if (FALSE) {
        prf <- predict(estimation[[2]][["fit.cox"]], newdata=dt2.a)
        test1 <- merge(mat[id==1], data.table(time=prf$forest$time.interest, chf0=prf$chf[1,], survival0=prf$survival[1,]), by="time")
        test1[, surv.C:=exp(-fit.cox0*chaz0)]
        test1[, surv.C2:=cumprod(1-dhaz0)]
        test1[, plot(surv.C, survival0)]
        test1[, all.equal(chaz0, chf0)]
        test1[, lines(surv.C2, survival0, col="blue")]
        abline(b=1, a=0)
    }
    
    if (FALSE) {
        plot(mat[id==1, time], mat[id==1, chaz1], type="l")
        test1 <- merge(mat[id==1], data.table(time=utimes, chf1.test=chf1[1,], dhf1.test=dhf1[,1]), by="time")
        test2 <- merge(mat[id==1], data.table(time=utimes, chf2.test=chf2[1,], dhf1.test=dhf2[,1]), by="time")
        #test1[, chf1.test:=na.locf(chf1.test)]
        #test2[, chf2.test:=na.locf(chf2.test)]
        lines(utimes, chf1[1, ])
        #lm(chf1~chf1.test, data=test1)
        lm(test1$chf1~ test1$chf1.test)
        lm(test2$chf2~ test2$chf2.test)
        #test1[, all.equal(chf1, chf1.test)]
    }
            ...
            if (any(estimation[[each]]$fit=="cox.hal") | any(estimation[[each]]$fit=="cox.hal.sl")) { #--- only testing.
                ...
                if (hal.screening) { #--- first screening
                    ...
                    if (hal.sl) {
                        ...
                        if (is.character(test.cve1[1])) {
                            cut.time.grid <- cut.time.grid[substr(test.cve1,1,1)!="E"]
                            test.cve1 <- test.cve1[substr(test.cve1,1,1)!="E"]
                        }

                        cut.time.min <- cut.time.grid[order(test.cve1, cut.time.grid)][1]

                        if (FALSE) plot(cut.time.grid, test.cve1)

                        if (verbose) print(paste0("cut.time.min = ", cut.time.min))
                        ...
                        if (verbose) print(paste0("cut.one.way.min = ", cut.one.way.min))
                        ...
                        cut.time.covar.grid <- cut.time.grid[cut.time.grid <= cut.time.min]#cut.time.covar.grid[cut.time.covar.grid <= cut.time.min]
                        ...
                        if (FALSE) plot(cut.time.covar.grid, test.cve2)
                        ...
                        time.covar <- expand.grid(grid.time = cut.time.grid[cut.time.grid <= cut.time.min],#cut.time.covar.grid,
                                                  grid.covar = cut.covar.grid)#(c(5:cut.one.way.min))[seq(1, length(c(5:cut.one.way.min)), length=min(5,length(c(5:cut.one.way.min))))])

                        test.cve4 <- sapply(1:nrow(time.covar),
                                            function(jj) fit.hal(covars=one.way.screening, dt=dt,
                                                                 cut.one.way=cut.one.way.min,
                                                                 time.var=time.var, cut.time=cut.time.min,
                                                                 mat=mat, delta.var="delta.obs", V=V,
                                                                 intervention=a, 
                                                                 ...
                                                                 cut.time.covar=time.covar[jj, "grid.time"],
                                                                 cut.one.way.time=time.covar[jj, "grid.covar"],
                                                                 return.cve=TRUE)$cve$min$cve)
                        ...
                        cut.time.covar.min.jj <- (1:nrow(time.covar))[order(test.cve4, time.covar[,"grid.time"], time.covar[,"grid.covar"])][1]

                        if (verbose) {
                            print(paste0("cut.time.covar.min = ", time.covar[cut.time.covar.min.jj, "grid.time"]))
                            print(paste0("cut.one.way.time.min = ", time.covar[cut.time.covar.min.jj, "grid.covar"]))
                        }
                                
                        #cut.time.covar.min <- (cut.time.covar:cut.time.min)[order(test.cve2, cut.time.covar:cut.time.min)][1]
                        ...
                        if (FALSE) plot(cut.two.way.grid, test.cve5)
                        ...
                        if (FALSE) {
                            ...
                            test <- fit.hal(covars=one.way.screening, dt=dt, cut.one.way=15,
                                            time.var=time.var, cut.time=cut.time,
                                            mat=mat, delta.var="delta.obs", V=V,
                                            intervention=a, 
                                            verbose=verbose, delta.value=fit.delta,
                            ...
                            cve.tune.grid <- sapply(1:nrow(tune.grid), function(jj) {
                                ...
                                return(fit.hal(covars=covars, dt=dt, cut.one.way=tune.grid[jj,"cut.one.way"],
                                               ...
                                               cut.two.way=tune.grid[jj,"cut.two.way"], 
                                               penalize.treatment=FALSE,
                                               cut.time.cox=cut.time.cox, cut.time=0, time.var=time.var,
                                               delta.value=fit.delta,
                                               verbose=verbose, 
                                               predict=max(tau), treatment.prediction=A.name, 
                                               treatment=A.name,
                        ...
                        mat.try <- try(fit.hal(covars=one.way.screening, dt=dt, cut.one.way=cut.covars,
                                               time.var=time.var, cut.time=cut.time,
                                               mat=mat, delta.var="delta.obs", V=V,
                                               intervention=a, 
                                               verbose=verbose, delta.value=fit.delta,
                    ...
                    mat <- fit.hal(covars=covars, dt=dt, cut.one.way=15, time.var=time.var,
                                   cut.time=10,
                                   mat=mat, delta.var="delta.obs", V=V,
                                   intervention=a, 
                                   verbose=verbose, delta.value=fit.delta,
    ...
    if (mat[, min(prob.A)<cut.off.cens]) {
        ...
        if (dt[, length(unique(get(A.name)))]>2) {
            mat[prob.A<=cut.off.cens, Ht:=Ht*(prob.A)/cut.off.cens]
        } else {
    ...
    if (FALSE) {
        test1 <- merge(mat[id==1], data.table(time=utimes, cif1.test=cif1.1.test2[,1]), by="time")
        test1[, plot(F1.t, cif1.test)]
        lm(F1.t~cif1.test, data=test1)
        test2 <- merge(mat[id==1], data.table(time=utimes, cif2.test=cif2.1.test2[,1]), by="time")
        test2[, plot(F2.t, cif2.test)]
        lm(F2.t~cif2.test, data=test2)
        init.list
    }
    ...
    if ((length(tau)>1 & !iterative) | one.step | (length(target)>1 & !iterative)) {
        ...
        if (cr) {
            if (length(target)==length(update.list)) {
                ...
                if (length(update.list)==length(target)+1) {
                    names(update.list) <- c(paste0("F", target), "S")
                } else {
                    names(update.list) <- c("S")
                }
            }
        } else {
