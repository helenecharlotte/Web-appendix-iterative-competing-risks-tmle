...
sim.data <- function(n, loop.max=20, endoffollowup=30,
                     ...
                     firstevent=TRUE,
                     change.weibull=FALSE, # <- test
                     change.weibull.parameters = NULL, # <- test
                     censoring=TRUE,
    ...
    if (print.forms) {
        ...
        print(paste0("alphaT=", alphaT))
        print("Censoring hazard form: ")
        print(phiC)
        print(paste0("censoring.alpha: ", censoring.alpha))
        if (competing.risk) {
        ...
        print(paste0("scale parameter (eta) = ", eta))
        print(paste0("shape parameter (nu) = ", nu))
    }
    ...
    if (interaction.Atime) {
        if (interaction.AL) {
            Lambda.inv <- function(u, t, A, L1, L2, L3, nu, eta, phi) {
            ...
            if (!change.weibull) {
                ...
                Lambda.inv <- function(u, t, A, L1, L2, L3, nu, eta, phi, nu2=nu, eta2=eta) {
                    return( rowSums(cbind((u <= (eta*phi(t, A, L1, L2, L3, betaA, betaL))*t0^{nu}) *
                                          (( (u + eta*phi(t, A, L1, L2, L3, betaA, betaL)*t^{nu}) /
                                             (eta*phi(t, A, L1, L2, L3, betaA, betaL)) )^{1/nu} - t),
                    (u > (eta*phi(t, A, L1, L2, L3, betaA, betaL))*t0^{nu}) *
                    (( (u - (eta2*phi(t, A, L1, L2, L3, betaA, betaL))*t0^{nu2} +
                        eta2*phi(t, A, L1, L2, L3,-0.45*betaA, betaL)*t0^{nu2}) /
                       (eta2*phi(t, A, L1, L2, L3,-0.45*betaA, betaL)) )^{1/nu2} - t)), na.rm=TRUE) )
                }
            }
        }
        Lambda.inv1 <- function(u, t, A, L1, L2, L3, nu, eta, phi) {
    ...
    loop.fun <- function(k, Tprev) {
        ...
        if (change.weibull) {
            ...
            U1.A1 <- -log(runif(n)); U1.A0 <- -log(runif(n)); U1 <- -log(runif(n)); U2 <- -log(runif(n)); U0 <- -log(runif(n))

            phiT1 <- function(t, A, L1, L2, L3, betaA, betaL) {
                phiT(t, A, L1, L2, L3, betaA, betaL)*exp(-alphaT)
            }

            summary(Tout1.A1 <- Lambda.inv(U1.A1, Tprev, 0, L1, L2, L3,
                                           ...
                                           phiT1) + Tprev)
            summary(Tout1.A0 <- Lambda.inv(U1.A0, Tprev, 0, L1, L2, L3,
            ...
            summary(Tout1 <- A*Tout1.A1 + (1-A)*Tout1.A0 + Tprev)
            # summary(Tout1.test1 <- A*Tout1.A1.test1 + (1-A)*Tout1.A0.test1 + Tprev)
            summary(Tout1.A0.test1 <- rweibull(n, shape=nu, scale=(eta*exp(alphaT-1.2*L1^2))^(-1/nu)))
            summary(Tout1.A0.test2 <- Lambda.inv(U1.A0, Tprev, 0, L1, L2, L3, nu, eta, phiT))
            summary(Tout1.A0)

            summary(Tout1.A1.test2 <- Lambda.inv(U1.A1, Tprev, 1, L1, L2, L3, nu, eta, phiT))
            summary(Tout1.A1)

            summary(Tout2 <- Lambda.inv(U2, Tprev, A, L1, L2, L3, nu, eta, phiT2) + Tprev)
            summary(Tout2.test <- rweibull(n, shape=nu, scale=(eta*exp(-0.2+0.7*L1-0.4*L3+betaA2*A))^(-1/nu)))
            
            summary(Tout0 <- Lambda.inv(U0, Tprev, A, L1, L2, L3, nu, eta, phiC) + Tprev)
            summary(Tout0.test <- rweibull(n, shape=nu, scale=(eta*exp(-0.1+censoring.alpha))^(-1/nu)))
            
            #-- REMOVE
            if (FALSE) {
                Tout1 <- Lambda.inv(U1, Tprev, A, L1, L2, L3, nu, eta, phiT) + Tprev
                Tout2 <- Lambda.inv(U2, Tprev, A, L1, L2, L3, nu, eta, phiT2) + Tprev
                Tout0 <- Lambda.inv(U0, Tprev, A, L1, L2, L3, nu, eta, phiC) + Tprev
            }

            Tout <- #sapply(1:length(Tout1), function(ii) min(Tout1[ii], Tout2[ii], Tout0[ii]))
                apply(cbind(Tout1, Tout2, Tout0), 1, min)
            which <- 1*(Tout1 == Tout) + 2*(Tout2 == Tout)
                
        } else {
        
            Tout <- Lambda.inv(U, Tprev, A, L1, L2, L3, nu, eta, phi) + Tprev
    ...
    #if (change.weibull) loop.max <- 2

    #-- run simulations: 
    for (k in 1:loop.max) {
        Tlist[[k+1]] <- loop.fun(k, Tlist[[k]][,1])
        #if (k == 1  & change.weibull) {
        #    Tlist[[k+1]][, "time"] <-
        #        sapply(Tlist[[k+1]][, "time"], function(tt) min(tt, t0))
        #} else if (change.weibull) {
        #    Tlist[[k]][Tlist[[k]][, "time"] == t0,] <-
        #        Tlist[[k+1]][Tlist[[k]][, "time"] == t0,] 
        #}
    }
